...
if model_type == "gru":
    ...
    config.n_classes = opt.num_classes 
    config.timestep = opt.sequence_length
    model_gru = GRU(config=config)
    model = model_gru.build()
    print(model.summary())
...
if model_type == "bilstm":
    ...
    config.n_classes = opt.num_classes 
    config.timestep = opt.sequence_length
    model_bilstm = BiLSTM(config=config)
    model = model_bilstm.build() 
    print(model.summary())
...
if model_type == 'adaptive_gru':
    from model.adaptive_gru import AdaptiveGRU
    from config.adaptive_lstm import Config
    config  = Config
    config.n_classes = opt.num_classes
    config.timestep  = opt.sequence_length
    ...
    config.kernel_size = opt.kernel_size
    config.normalizer = opt.normalizer
    model_adaptive_gru = AdaptiveGRU(config=config)
    model = model_adaptive_gru.build()
    print(model.summary())

if model_type == 'adaptive_bilstm':
    from model.adaptive_bilstm import AdaptiveBiLSTM
    from config.adaptive_lstm import Config
    config  = Config
    config.n_classes = opt.num_classes
    config.timestep  = opt.sequence_length
    ...
    config.kernel_size = opt.kernel_size
    config.normalizer = opt.normalizer
    model_adaptive_bilstm = AdaptiveBiLSTM(config=config)
    model = model_adaptive_bilstm.build()  
    print(model.summary())      
